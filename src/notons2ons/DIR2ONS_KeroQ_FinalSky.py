...
def title_info():
	return {
		...
		'notes': [
			...
			'改ページの位置が違う場面がある',
			'セーブ/ロード画面は簡略化',
			'起動時のケロQロゴ&赤背景の説明画像(Macromediaロゴと18歳未満は～的な文)が無い',
			'初回起動時のみ出てくる専用のタイトル画面(青空背景に"スタート"、"終了"のみの画面)が無い',
			'マップ移動などの繰り返し選択肢が出る場面で、本来出ない選択肢が出る',
			'クリアモードでのシナリオ選択時、個別ルートの途中から開始することができない',
		]
	}
