...
def title_info():
	return {
		...
		'notes': [
			'クリック待ちと改ページ待ちの構文の違いが不明だったので、強制的にすべての文章が改ページ待ちの状態に',
			'上記仕様により画面におけるスペースが余る＆低解像度機種での文字潰れを防ぐため文字は原作より大きめ',
			'ONScripterの縦書き表示は安定性が低いため、文字表示は横書き固定、また文字サイズも変更不可',
			'共通ルートの好感度調整が機能していないため、個別ルート分岐時の選択肢は何選んでも全開放',
			'その他セーブ/ロード画面は超簡略化、CG/回想モードなどに関しても未実装',
		]
	}
