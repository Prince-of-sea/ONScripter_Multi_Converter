...
def refresh_state(sender, app_data, user_data):
    dpg.set_viewport_title(
        f'ONScripter Multi Converter for {user_data[0]} ver.{user_data[1]}')
...
def gui_main(version: str, charset_param: str, hw_key: str, input_dir_param: str, output_dir_param: str, title_setting_param: str, value_setting_param: str):
    ...
    if (charset_param == 'cp932'):
        title_setting_list += list(get_titledict().keys())  # 個別設定はcp932時のみ有効
    ...
    with dpg.window(label='Main Window', tag='Main Window', no_resize=True) as window:
        with dpg.menu_bar():
            with dpg.menu(label=i18n.t('ui.label_settings')):
                dpg.add_menu_item(label=i18n.t('ui.label_cpu_usage_reduction_mode'),
                                  check=True, default_value=False, tag='lower_cpu_usage')
                with dpg.menu(label=i18n.t('ui.label_hardware_selection')):
                    for hwk, hwv in hardwarevalues_full.items():
                        if (charset_param in hwv['values_ex']['support_charset']):
                            dpg.add_menu_item(label=hwv['values_ex']['hardware_full'],
                                              user_data=(hwk, version), callback=refresh_state)

                dpg.add_menu_item(label=i18n.t('ui.label_end'),
                                  callback=close_dpg)
            ...
            with dpg.menu(label=i18n.t('ui.label_tools')):
                ...
                dpg.add_menu_item(label=i18n.t('ui.label_open_garbro'),
                                  callback=open_garbro)

            with dpg.menu(label=i18n.t('ui.label_about')):
                dpg.add_menu_item(label=i18n.t('ui.label_open_web'),
                                  callback=open_repositorieslink)
                dpg.add_menu_item(label=i18n.t('ui.label_copyrights'),
                                  user_data=(version), callback=copyrights)
                if get_meipass('licenses_py.txt').is_file():
                    dpg.add_menu_item(label=i18n.t('ui.label_license'),
                                      callback=open_licensespy)
        ...
        with dpg.table(header_row=False, borders_innerH=False, borders_innerV=False):
            ...
            with dpg.table_row():
                ...
                dpg.add_button(label='Browse', callback=open_input,
                               tag='input_browse_btn')
                dpg.add_button(label=i18n.t('ui.label_select_from_list'), user_data=(
                    charset_param), callback=open_select, tag='input_select_btn')
            ...
            with dpg.table_row():
                ...
                dpg.add_button(label='Browse', callback=open_output,
                               tag='output_browse_btn')
                dpg.add_button(label=i18n.t('ui.label_set_to_default'),
                               callback=desktop_output, tag='output_desktop_btn')

            with dpg.table_row():
                dpg.add_text(i18n.t('ui.label_individual_settings'))
                dpg.add_combo(
                    tag='title_setting',
                        default_value=(get_titlesettingfull(
                            title_setting_param) if charset_param == 'cp932' else 'None'),
                ...
                with dpg.child_window(height=195, border=False):
                    with dpg.tree_node(label=i18n.t('ui.label_basic_settings'), default_open=True):
                        with dpg.group(horizontal=True):
                            dpg.add_text(
                                i18n.t('ui.label_image_preferred_format'))
                            dpg.add_radio_button(
                                items=(i18n.t('var.high_resolution_priority'),
                                       i18n.t('var.low_resolution_priority')),
                        ...
                        with dpg.group(horizontal=True):
                            dpg.add_checkbox(label=i18n.t('ui.label_png_reduction'),
                                             default_value=values_default['img_pngquantize_chk'], tag='img_pngquantize_chk')
                            dpg.add_combo(
                    ...
                    with dpg.tree_node(label=i18n.t('ui.label_advanced_settings'), default_open=True):
                        ...
                        with dpg.tree_node(label=i18n.t('ui.label_mp3_conversion'), default_open=True):
                            with dpg.table(header_row=False, borders_innerH=False, borders_innerV=False):
                                ...
                                with dpg.table_row():
                                    dpg.add_text(
                                        i18n.t('ui.label_bgm_cutoff_frequency'))
                                    dpg.add_combo(
                                        label='Hz',
                                        items=(
                                            '18000',
                                ...
                                with dpg.table_row():
                                    dpg.add_text(
                                        i18n.t('ui.label_se_cutoff_frequency'))
                                    dpg.add_combo(
                                        label='Hz',
                                        items=(
                                            '18000',
                ...
                with dpg.child_window(height=195, border=False):
                    with dpg.tree_node(label=i18n.t('ui.label_basic_settings'), default_open=True):
                        with dpg.group(horizontal=True):
                            ...
                            dpg.add_radio_button(
                                items=(i18n.t('var.numbered_images'), 'MJPEG',
                                       'MP4', i18n.t('var.do_not_convert')),
                    ...
                    with dpg.tree_node(label=i18n.t('ui.label_advanced_settings'), default_open=True):
                        with dpg.tree_node(label=i18n.t('ui.label_video_numbered'), default_open=True):
                            ...
                            with dpg.group(horizontal=True):
                                dpg.add_text(
                                    i18n.t('ui.label_video_numbered_jpeg_quality'))
                                dpg.add_slider_int(
                                    label='',
                        ...
                        with dpg.tree_node(label=i18n.t('ui.label_video_mp4'), default_open=True):
                            ...
                            with dpg.group(horizontal=True):
                                dpg.add_text(
                                    i18n.t('ui.label_video_audio_bitrate'))
                                dpg.add_combo(
                                    label='kbps',
                                    items=(
                                        '192',
                ...
                with dpg.child_window(height=195, border=False):
                    with dpg.tree_node(label=i18n.t('ui.label_basic_settings'), default_open=True):
                        with dpg.tree_node(label=i18n.t('ui.label_file'), default_open=True):
                            ...
                            with dpg.table(header_row=False, borders_innerH=False, borders_innerV=False):
                                ...
                                with dpg.table_row():
                                    dpg.add_text(
                                        i18n.t('ui.label_image_compression'))
                                    dpg.add_combo(
                                        label='',
                                        items=('arc.nsa', 'arc1.nsa', 'arc2.nsa',
                                               i18n.t('var.do_not_compress')),
                                ...
                                with dpg.table_row():
                                    dpg.add_text(
                                        i18n.t('ui.label_bgm_compression'))
                                    dpg.add_combo(
                                        label='',
                                ...
                                with dpg.table_row():
                                    dpg.add_text(
                                        i18n.t('ui.label_se_compression'))
                                    dpg.add_combo(
                                        label='',
                                        items=('arc.nsa', 'arc1.nsa', 'arc2.nsa', i18n.t('var.do_not_compress')),
                                ...
                                with dpg.table_row():
                                    dpg.add_text(
                                        i18n.t('ui.label_numvideo_compression'))
                                    dpg.add_combo(
                                        label='',
                                        items=('arc.nsa', 'arc1.nsa', 'arc2.nsa', i18n.t('var.do_not_compress')),
                        ...
                        with dpg.tree_node(label=i18n.t('ui.label_onsini'), default_open=True):
                            with dpg.table(header_row=False, borders_innerH=False, borders_innerV=False):
                                ...
                                with dpg.table_row():
                                    dpg.add_text(
                                        i18n.t('ui.label_onsini_expansion'))
                                    dpg.add_combo(
                                        label='',
                                        items=(i18n.t('var.no_expansion'),
                                               i18n.t('var.maintain_ratio'), i18n.t('var.full_size')),
                    ...
                    with dpg.tree_node(label=i18n.t('ui.label_advanced_settings'), default_open=True):
                        with dpg.tree_node(label=i18n.t('ui.label_0txt'), default_open=True):
                            with dpg.group(horizontal=True):
                                dpg.add_text(
                                    i18n.t('ui.label_0txt_nbzconversion'))
                                dpg.add_radio_button(
                                    items=(
                            ...
                            with dpg.table(header_row=False, borders_innerH=False, borders_innerV=False):
                                ...
                                with dpg.table_row():
                                    dpg.add_text(
                                        i18n.t('ui.label_0txt_avitompegplay'))
                                    dpg.add_combo(
                                        items=(
                                ...
                                with dpg.table_row():
                                    dpg.add_text(
                                        i18n.t('ui.label_0txt_noscreenshot'))
                                    dpg.add_combo(
                                        items=(
                                            i18n.t('var.use_function_override'),
                                            i18n.t('var.use_regex_replace'),
        ...
        with dpg.group(horizontal=True):
            dpg.add_progress_bar(
                default_value=0, tag='progress_bar', overlay='0%')
            ...
            # 関数convertにhardwareを飛ばすため仕方なくこんな感じに
            dpg.add_text(default_value=hw_key, tag='hardware')
