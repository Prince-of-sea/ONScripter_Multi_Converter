        ...
        if f_path.is_file():  # ファイル(=ディレクトリではない)なら処理
            ...
            if (f_path_re in values_ex['vidlist']) or (fc in ['OGV', 'MP4', 'MPEG', 'AVI']):
                ...
                if (vid_movfmt_radio == i18n.t('var.numbered_images')):
                    if (etc_filecomprenban_nsa == 'arc.nsa'):
                        ...
                        raise ValueError(
                            i18n.t('ui.Sequential_image_compression_not_selected'))
            ...
            # [画像]
                ...
                if (values_ex['imgdict'].get(f_path_re) != None):
                    ...
                    if d.get('part'):
                        ...
                        if values['img_multi_chk']:
                            d_part = str(values['img_multi_num'])
                        else:
                            d_part = '1'
                ...
                # アルファブレンドのためx2分割
                if (fc in ['JPEG', 'BMP']) and (d_trans == 'a'):
                    d_part = str(int(d_part) * 2)
                ...
                if (etc_filecompimg_nsa == 'arc.nsa'):
                    ...
                    raise ValueError(
                        i18n.t('ui.Image_compression_not_selected'))
            ...
            # [音楽]
                ...
                if (f_path_re in values_ex['bgmlist']) or ('bgm' in str(f_path_re).lower()) or (re.fullmatch(r'cd[\/\\]track[0-9]{2}\.[A-z0-9]+', str(f_path_re).lower())):
                    ...
                    if (etc_filecompbgm_nsa == 'arc.nsa'):
                        ...
                        raise ValueError(
                            i18n.t('ui.BGM_compression_not_selected'))
                    ...
                    if (etc_filecompse_nsa == 'arc.nsa'):
                        ...
                        raise ValueError(
                            i18n.t('ui.SE_compression_not_selected'))
            ...
            # [他]
                ...
                if (etc_filecompimg_nsa == 'arc.nsa'):
                    ...
                    raise ValueError(
                        i18n.t('ui.Other_compression_not_selected'))
...
def tryconvert(values: dict, values_ex: dict, f_dict: dict, f_path_re: Path, converted_dir: Path):
    ...
    try:
        match f_dict['fileformat']:  # ファイルフォーマットによる分岐
            ...
            # csvに書く用、多分出ない
            case _: raise Exception('Unknown format. (messages from converter.)')

    except Exception as e:
        dt_now = datetime.datetime.now()
        errmsg = f'{dt_now.strftime('%Y/%m/%d %H:%M:%S.%f')}\t{f_path_re}\t{e}\n'
        with open(Path(converted_dir / ('ERROR' + dt_now.strftime('%Y%m%d%H%M%S%f') + str(f_path_re.stem).upper()[:20])), 'w', encoding='utf-8') as s:
            s.write(errmsg)
