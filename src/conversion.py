...
def convert_files(values: dict, values_ex: dict, cnvset_dict: dict, extracted_dir: Path, converted_dir: Path, useGUI: bool):
    ...
    for f_path_re, f_dict in cnvset_dict.items():
        # 元nbzにフラグ付け
        f_dict['nbz'] = bool(f_path_re.with_suffix('.nbz')
                             in values_ex['nbzlist'])

        # パスの代入
        f_dict['extractedpath'] = extracted_dir / f_path_re
        f_dict['convertedpath'] = converted_dir / \
            f_dict['comp'] / 'arc_' / f_path_re
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=num_workers) as executor:
        ...
        for f_path_re, f_dict in cnvset_dict.items():
            futures.append(executor.submit(tryconvert, values,
                           values_ex, f_dict, f_path_re, converted_dir))
        ...
        for p in arc_dir.glob('**/*'):
            if p.is_file():  # ファイルのみを取得
                if (not dir_flag):  # フラグが立っていなかったら
                    ...
                    # 容量が2000MBより大きいまたは20000ファイルより多いなら
                    if (dir_size > 2097152000) or (dir_cnt > 20000):
                        dir_flag = True  # フラグを立てる

                if (dir_flag):  # フラグが立っていたら - 乗った瞬間からこっちの処理走らせるためelse未使用
                    p_moved = Path(converted_dir / ex_dir_name /
                                   'arc_' / p.relative_to(arc_dir))  # 移動先パス
                    p_moved.parent.mkdir(
                        parents=True, exist_ok=True)  # 移動先ディレクトリ作成
                    shutil.move(p, p_moved)  # ファイルを移動する

    # arcダミー処理
    dummy_dir_flag = False
    # arc2からarcまで(3以降は順番に生成されるはずのため歯抜けは起きないはず)
    for dummy_dir_num in range(2, -1, -1):

        if (dummy_dir_num > 0):
            dummy_dir_name = f'arc{dummy_dir_num}'
        ...
        if (not dummy_dir_flag) and (dummy_dir.is_dir()):
            dummy_dir_flag = True

        # フラグが立っているのにarcが存在しないなら
        elif (dummy_dir_flag) and (not dummy_dir.is_dir()):
            dummy_dir.mkdir(parents=True)  # とりあえずarc作って
            with open(Path(dummy_dir / '.dummy'), 'wb') as s:
                s.write(b'\xff')  # ダミー突っ込んどく(ここ意味があるのか不明、未検証)

    # エラーログ収集
    allerrlog = ''
    for errlogpath in converted_dir.glob('ERROR*'):
        with open(errlogpath, 'r', encoding='utf-8') as er:
            allerrlog += er.read()
...
def convert_start(values):
    ...
    try:
        ...
        # 必要ソフトチェック
        if not exist_env('ffmpeg'):
            raise FileNotFoundError(i18n.t('ui.ffmpeg_not_found'))
        if not exist_env('ffprobe'):
            raise FileNotFoundError(i18n.t('ui.ffprobe_not_found'))
        if not exist_all(required_soft_list):
            raise FileNotFoundError(i18n.t('ui.required_software_not_found'))
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            # 最終的な出力はこれ
            result_dir = Path(
                values['output_dir'] / Path(f'result_{values['hardware']}_{values['input_dir'].name}'))
            ...
            configure_progress_bar(0.005,
                                   i18n.t('ui.Progress_get_hardware_settings'), useGUI)
            values_ex = gethardwarevalues(values['hardware'], 'values_ex')

            # 並列処理用スレッド設定
            if (values.get('lower_cpu_usage')):
                values_ex['num_workers'] = math.ceil(os.cpu_count() / 4) 
                
            else:
                # low時スレッド数/4繰り上げ、通常時スレッド数+4(初期値)
                values_ex['num_workers'] = (os.cpu_count() + 4)
            ...
            if values['title_setting'] in titledict.keys():
                ...
                # 個別変換用事前変換
                configure_progress_bar(0.01,
                                       i18n.t('ui.Progress_pre_conversion'), useGUI)
                pre_convert(values, values_ex, pre_converted_dir)

                # 画像解像度がスクリプト側と一致しない時用
                values_ex['input_resolution'] = title_info.get(
                    'input_resolution')

                # アイコン取得用exeのパスを取得
                values_ex['icon_exe_path'] = get_iconexepath(
                    values, title_info)
            ...
            if (values['vid_movfmt_radio'] == i18n.t('var.numbered_images')):
                configure_progress_bar(0.02,
                                       i18n.t('ui.Progress_set_sequential_images'), useGUI)
                values_ex['renbanresper'] = getvidrenbanres(values)

            # 0.txtのテキスト取得
            configure_progress_bar(0.023,
                                   i18n.t('ui.Progress_get_scenario_text'), useGUI)
            values_ex['0txtscript'] = onsscript_decode(values)

            # 0.txtのテキストから各種情報取得
            configure_progress_bar(0.026,
                                   i18n.t('ui.Progress_get_scenario_info'), useGUI)
            values_ex = onsscript_check(values, values_ex)

            # 0.txtのコメントアウト削除
            if values['etc_0txtremovecommentout_chk']:
                configure_progress_bar(0.029,
                                       i18n.t('ui.Progress_remove_scenario_comments'), useGUI)
                values_ex['0txtscript'] = remove_0txtcommentout(values_ex)

            # nsa/sar展開
            configure_progress_bar(0.03,
                                   i18n.t('ui.Progress_extract_archive'), useGUI)
            values_ex = extract_nsa(
                values, values_ex, extracted_dir, useGUI)  # 進捗 0.03→0.045

            # 画像/音楽/動画/その他のファイルを仕分け
            configure_progress_bar(0.048,
                                   i18n.t('ui.Progress_set_extracted_data'), useGUI)
            ...
            # 変換済ファイルをnsaに圧縮
            configure_progress_bar(0.95,
                                   i18n.t('ui.Progress_rebuild_archive'), useGUI)
            compressed_nsa(converted_dir, compressed_dir,
                           useGUI)  # 進捗 0.95→0.98
            ...
            # 機種固有コンフィグファイル作成
            configure_progress_bar(0.98,
                                   i18n.t('ui.Progress_create_config'), useGUI)
            ...
            configure_progress_bar(0.982,
                                   i18n.t('ui.Progress_write_converted_scenario'), useGUI)
            create_0txt(values, values_ex, compressed_dir)

            # 完成品移動
            configure_progress_bar(0.985,
                                   i18n.t('ui.Progress_move_all_data'), useGUI)
            debug_copy(values, compressed_dir)
            result_move(result_dir, compressed_dir)

            # まもなく完了します
            configure_progress_bar(0.99,
        ...
        cnvmsg = i18n.t('ui.Conversion_complete_time').replace(
            r'{m}', m).replace(r'{s}', s)

        message_box(i18n.t('ui.Conversion_complete_title'),
                    cnvmsg, 'info', useGUI)

        if (useGUI):
            configure_progress_bar(
                1, i18n.t('ui.Conversion_complete_title'), True)
