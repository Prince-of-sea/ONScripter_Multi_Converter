...
def ask_decode_nscriptdat(sender, app_data, charset):
    with dpg.mutex():
        with dpg.window(label=i18n.t('ui.label_decoding_nscript_dat'), modal=True) as msg_ask:
            ...
            with dpg.group(horizontal=True):
                dpg.add_button(label='OK', user_data=(
                    msg_ask, charset, True), callback=decode_nscriptdat)
                dpg.add_button(label='Cancel', user_data=(
                    msg_ask, charset, False), callback=decode_nscriptdat)
    dpg.split_frame()
    dpg.set_item_pos(msg_ask,
                     [dpg.get_viewport_client_width() // 2 - dpg.get_item_width(msg_ask) // 2,
                      dpg.get_viewport_client_height() // 2 - dpg.get_item_height(msg_ask) // 2])
...
def decode_nscriptdat(sender, app_data, user_data):
    ...
    root = tkinter.Tk()
    root.withdraw()
    _path = filedialog.askopenfilename(filetypes=[(
        'NScripter script', 'nscript.dat;*.scp')])  # どうせ同じなので旧Scripterもできるようにしておく
    root.destroy()
    ...
    with open(txtpath, 'w', encoding=charset, errors='ignore') as s:
        s.write(openread0x84bitxor(_path, charset))

    message_box(i18n.t('ui.Completed'),
                i18n.t('ui.Decrypted_nscript_dat'), 'info', True)
    return


def open_garbro():
    if exist('GARbro_GUI'):
        sp.Popen([location('GARbro_GUI')])
    else:
        message_box(i18n.t('ui.Warning'),
                    i18n.t('ui.Not_found_GARbro_GUI'), 'warning', True)
    return
...
def copyrights(sender, app_data, user_data):
    message_box(
        'copyrights', f'ONScripter Multi Converter ver.{user_data}\n(C) 2021-2025 Prince-of-sea / PC-CNT / RightHand', 'info', True)
    return
    ...
    with dpg.window(label=i18n.t('ui.Installed_game_list'), height=360, width=628, modal=True, no_move=True) as opsel:
        ...
        with tempfile.TemporaryDirectory() as icotemp_dir:
            ...
            for d in programs_list:
                ...
                # アイコン作成
                # 数が多ければ並行処理のほうが速いだろうけどめんどいのでそのまま
                exepath2icon(d['exe_path'], d['icon_path'])
                ...
                with dpg.group(horizontal=True, height=32):
                    ...
                    # ボタン表示
                    dpg.add_button(label=f'{d['name']}\t({d['exe_path'].parent}){' '*99}',
                                   user_data=(opsel, d), width=560, callback=open_select_main)
...
def open_select_main(sender, app_data, user_data):
    ...
    if d['overwrite_title_setting']:
        dpg.set_value('title_setting', d['overwrite_title_setting'])
    else:
...
def ask_convert_start(sender, app_data, user_data):
    ...
    if dpg.get_value('title_setting') in titledict.keys():
        ...
        configure_progress_bar(
            0, i18n.t('ui.Progress_check_individual_settings_conversion'), True)
        ...
        for hw_k, hw_v in gethardwarevalues_full().items():
            if (hw_k == 'PSP'):
                hw_s = '可'  # PSPなら(埋め込み昆布は全部動くように作ってるはずなので)ok
            elif (not hw_v['values_ex']['aspect_4:3only']):
                hw_s = '可'  # 4:3専用機ではないならok
            elif is_43:
                hw_s = '可'  # 作品が4:3ならok
            else:
                hw_s = '不可'
        ...
        if i18n.t('var.default_language') != 'ja':
            s = (
                '=================================================================\n'
            ...
            with dpg.window(label=i18n.t('ui.label_check_individual_settings_conversion').replace(r'{title}', title), modal=True, no_close=True, no_move=True) as msg_askconv:
                ...
                with dpg.group(horizontal=True):
                    dpg.add_button(label='OK',
                                   user_data=(msg_askconv, True, values), callback=askconv_callback)
                    dpg.add_button(label='Cancel',
                                   user_data=(msg_askconv, False, values), callback=askconv_callback)
        dpg.split_frame()
    # Noneなら確認を飛ばして変換開始
    else:
...
def askconv_callback(sender, app_data, user_data):
    ...
    if user_data[1]:
        ...
        configure_progress_bar(0,
                               i18n.t('ui.Progress_cancel_conversion'), True)
    return
