...
def convert_video_renban(values: dict, values_ex: dict, f_dict: dict):
	...
	#縦横指定
	if hardware == 'PSVITA':
		pw = math.ceil(540 / values_ex['output_resolution'][1] * values_ex['output_resolution'][0])#基本960or725
		ph = 544
	...
	if getvidaudio(extractedpath):
		...
		shutil.move(dummy_dict['convertedpath'], Path(convertedpath / 'audio.mus'))
...
def convert_video_renban2_main(values: dict, imgpath: Path):
	...
	match values['vid_renbanfmt_radio']:
		case 'JPEG':
			convertedpath2 = imgpath.parent / (str(imgpath.stem)[1:] + '.jpg')#名前最初の"_"切り取り

			#BytesIO/mozjpeg経由保存
			io_img = BytesIO()
			img.save(io_img, format='JPEG', quality=vid_renbanjpgquality_bar)
			io_img.seek(0)
			with open(convertedpath2, 'wb') as c: c.write(mozj.optimize(io_img.read()))
			
		case 'PNG':
			convertedpath2 = imgpath.parent / (str(imgpath.stem)[1:] + '.png')#名前最初の"_"切り取り

			#imagequant/BytesIO/ZopfliPNG経由保存
			if vid_renbanpngquantize_chk: quant_img = iq.quantize_pil_image(img, dithering_level=1.0, max_colors=vid_renbanpngquantize_num, min_quality=0, max_quality=100)
			else: quant_img = img
			io_img = BytesIO()
