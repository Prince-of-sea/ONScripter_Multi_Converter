...
def checknbz_nsa(p: Path):
    ...
    for f in re.findall(r'\[[A-F0-9]{8}\]\s+[0-9]+\s+(.+?)\n', garbrolist_raw):
        if str(Path(f).suffix).lower() == '.nbz':
            l.append(Path(f))
            ...
            with concurrent.futures.ThreadPoolExecutor() as executor:
                ...
                for nsasar_path in nsasar_pathlist:
                    ...
                    futures.append(executor.submit(
                        extract_archive_garbro, nsasar_path, deftempnsasar_path))  # nsaやsarを展開

                for i, ft in enumerate(concurrent.futures.as_completed(futures)):
                    configure_progress_bar(
                        # 進捗 0.03→0.042
                        0.03 + float(i / len(nsasar_pathlist) * 0.012), '', useGUI)
            ...
            for nsasar_path in nsasar_pathlist:
                ...
                for f_path in deftempnsasar_path.glob('**/*'):
                    if f_path.is_file():  # ファイル(＝ディレクトリではない)なら処理
                        f_movedpath = Path(
                            # 念の為全部小文字に
                            str(extracted_dir / f_path.relative_to(deftempnsasar_path)).lower())
                        f_movedpath.parent.mkdir(parents=True, exist_ok=True)
                        if (values['etc_fileexdll_chk']) and (str(f_movedpath.suffix) == '.dll'):
                            pass  # dll無視
                        elif (values['etc_fileexdb_chk']) and (str(f_movedpath.name) == 'thumbs.db'):
                            pass  # thumbs.db無視
                        else:
                            shutil.move(f_path, f_movedpath)
...
def compressed_nsa_main(arc_dir: Path, compressed_dir: Path):
    ...
    # コピーしたnsaedを走らせる
    try:
        sp.call([nsaedcopy_Path, (arc_dir / 'arc_')], **subprocess_args())
    except:
        dir_allmove((arc_dir / 'arc_'), compressed_dir)  # 異常終了時  - そのまま移動
    else:
        shutil.move((arc_dir / 'arc.nsa'), (compressed_dir /
                    (str(arc_dir.name) + '.nsa')))  # 正常終了時 - nsa移動
...
def compressed_nsa(converted_dir: Path, compressed_dir: Path, useGUI: bool):
    ...
    with concurrent.futures.ThreadPoolExecutor() as executor:
        ...
        for arc_dir in converted_dir.glob('arc*'):
            futures.append(executor.submit(
                compressed_nsa_main, arc_dir, compressed_dir))

        for i, ft in enumerate(concurrent.futures.as_completed(futures)):
            configure_progress_bar(
                # 進捗 0.95→0.98
                0.95 + float(i / len(list(converted_dir.glob('arc*'))) * 0.03), '', useGUI)

        concurrent.futures.as_completed(futures)

    # 圧縮しないやつが存在する場合移動
    no_comp_dir = (converted_dir / 'no_comp' / 'arc_')
    if no_comp_dir.exists():
        dir_allmove(no_comp_dir, compressed_dir)
