...
def get_titledict():
    ...
    d['[FrontWing]魔界天使ジブリール (Vista対応版)'] = notons2ons.QLIE2ONS_fw_djib.title_info()
    d['[Liar-soft]SEVEN-BRIDGE'] = notons2ons.RSC2ONS_Liar_SB.title_info()
    d['[minori]はるのあしおと'] = notons2ons.HARU2ONS.HARU2ONS.title_info()
    ...
    d['[アパタイト]祖母シリーズ汎用 (2017〜2020)'] = notons2ons.KIRIKIRI2ONS_APTIT_sobo.title_info()
    d['[アパタイト]祖母シリーズHD汎用 (2021～2022)'] = notons2ons.KIRIKIRI2ONS_APTIT_soboHD.title_info()
    d['[いつものところ]Kanoso ～思い出を壊す物語～'] = notons2ons.SC32ONS_itsumono_kanoso.title_info()
...
def pre_convert(values: dict, values_ex: dict, pre_converted_dir: Path):
    ...
    match values['title_setting']:
        ...
        case '[FrontWing]魔界天使ジブリール (Vista対応版)': notons2ons.QLIE2ONS_fw_djib.main(values, values_ex, pre_converted_dir)
        case '[Liar-soft]SEVEN-BRIDGE': notons2ons.RSC2ONS_Liar_SB.main(values, values_ex, pre_converted_dir)
        case '[minori]はるのあしおと': notons2ons.HARU2ONS.HARU2ONS.main(values, values_ex, pre_converted_dir)
        ...
        case '[アパタイト]祖母シリーズ汎用 (2017〜2020)': notons2ons.KIRIKIRI2ONS_APTIT_sobo.main(values, values_ex, pre_converted_dir)
        case '[アパタイト]祖母シリーズHD汎用 (2021～2022)': notons2ons.KIRIKIRI2ONS_APTIT_soboHD.main(values, values_ex, pre_converted_dir)
        case '[いつものところ]Kanoso ～思い出を壊す物語～': notons2ons.SC32ONS_itsumono_kanoso.main(values, values_ex, pre_converted_dir)
