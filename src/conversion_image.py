...
def convert_image(values: dict, values_ex: dict, f_dict: dict):
    ...
    elif (not (input_img.mode in ['RGB', 'RGBA'])):
    ...
    if scale != (1, 1):
        ...
        if (input_img.size in [(72, 24), (24, 24)]):
            ...
            for name, b64file in cur_dict['default'].items():
                ...
                if np.array_equal(input_img_np, default_img_np):
                    ...
                    if (scale[0] < 0.5):
                        output_img_b64 = cur_dict['small'][name]
                    else:
                        output_img_b64 = cur_dict['big'][name]

            # b64が代入できた(≒カーソルが合致した)場合
            if output_img_b64:

                # 書き込んで
                with open(convertedpath, 'wb') as f:
                    f.write(base64.b64decode(output_img_b64))
        ...
        match d_trans:
            ...
            case _:
                if (input_img.mode == 'RGB'):
                    ...
                    raise ValueError(i18n.t('ui.Not_RGB_or_RGBA'))

        # 通常処理(カーソル比較モード抜けた後)
        output_img = Image.new(
            input_img.mode, (output_width, output_height), trans_px)
        ...
        for di in range(d_part):  # 枚数分繰り返す
            cropped_img = input_img.crop(
                (crop_width * di, 0, crop_width * (di + 1), input_img.height))  # 画像切り出し
            ...
            if (d_trans in ['l', 'r']) and (d_format in ['BMP', 'PNG']) and (input_img.height >= 4) and (input_img.height <= 32) and ((cropped_img.width / cropped_img.height) == int(cropped_img.width / cropped_img.height)):
                ...
                cropped_img = cropped_img.resize(
                    (output_crop_width - 1, output_height - 1), Image.Resampling.LANCZOS)
                msk_img = msk_img.resize(
                    (output_crop_width - 1, output_height - 1), Image.Resampling.NEAREST)
                ...
                # 塗りつぶし背景作成
                fill_img = Image.new(
                    'RGB', (output_crop_width, output_height), trans_px)

                # マスク切り出し
                cropped_img = Image.composite(cropped_img, fill_img, msk_img)
                output_img.paste(
                    cropped_img, (output_crop_width * di, 1))  # 結合用画像へ貼り付け

            else:
                cropped_img = cropped_img.resize(
                    (output_crop_width, output_height), Image.Resampling.LANCZOS)  # 縮小
                output_img.paste(
                    cropped_img, (output_crop_width * di, 0))  # 結合用画像へ貼り付け
        ...
        if (setwinimgbug) and (setwinpos) and (output_img.mode == 'RGBA'):
            ...
            output_fix_img = Image.new(
                'RGBA', (output_fix_width, output_fix_height), (0, 0, 0, 0))
    ...
    match d_format:
        ...
        case 'JPEG':
            ...
            io_img.seek(0)
            with open(convertedpath, 'wb') as c:
                c.write(mozj.optimize(io_img.read()))

        # PNG
        case 'PNG':
            # imagequant/BytesIO/ZopfliPNG経由保存
            if img_pngquantize_chk:
                quant_img = iq.quantize_pil_image(
                    output_img, dithering_level=1.0, max_colors=img_pngquantize_num, min_quality=0, max_quality=100)
            else:
                quant_img = output_img
            io_img = BytesIO()
        ...
        case _:
            raise ValueError(i18n.t('ui.Image_save_format_not_selected'))

    # 元nbz用コビー
    if (f_dict['nbz']) and (convertedpath.suffix.lower() != '.nbz'):
